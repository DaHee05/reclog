      ...
      <div className="max-w-lg mx-auto pb-24">
        ...
        <main className="px-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <h2 className="text-lg font-semibold text-foreground">{formatMonthYear(currentDate)}</h2>
          ...
          <div className="bg-card rounded-2xl p-4">
            <div className="grid grid-cols-7 gap-1 mb-2">
              {DAYS.map((day, index) => (
                <div key={day} className={cn('text-center text-sm font-medium py-2', index === 0 ? 'text-rose-400' : 'text-muted-foreground')}>
                  {day}
                </div>
              ))}
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? 'bg-primary text-primary-foreground font-semibold' : isToday ? 'bg-muted font-bold ring-2 ring-primary/30' : 'hover:bg-muted',
                      isSunday && !isSelected && 'text-rose-400'
                    )}
                  >
                    <span>{day}</span>
                    {hasRecords && (
                      <span className={cn('absolute bottom-1 w-1.5 h-1.5 rounded-full', isSelected ? 'bg-primary-foreground' : 'bg-primary')} />
                    )}
                  </button>
                );
              })}
          ...
          {selectedDate && (
            <div className="mt-4 bg-card rounded-2xl p-4 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => { setSelectedDate(null); setSelectedRecords([]); }} className="p-1.5 rounded-full hover:bg-muted">
              ...
              {selectedRecords.length > 0 ? (
                <div className="space-y-3">
                  {selectedRecords.map((record) => (
                    <Link key={record.id} href={`/record/${record.id}`} className="flex gap-3 p-2 -mx-2 rounded-xl hover:bg-muted transition-colors">
                      {record.images[0] && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <p className="font-medium text-foreground truncate">{record.title || record.location}</p>
                        </div>
