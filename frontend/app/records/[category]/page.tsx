...
// Feed View Component (Instagram-style)
...
export default function CategoryRecordsPage() {
  ...
  const handleJoinWithCode = () => {
    if (joinCode.trim().length === 6) {
      alert(`코드 "${joinCode}"로 공유 공간에 참여했습니다! 이제 글을 작성할 수 있습니다.`);
      setJoinCode('');
  ...
  const filteredRecords = useMemo(() => {
    ...
    // Filter by search query (hashtag search)
    if (searchQuery.trim()) {
          ...
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  href={`/record/new?category=${category}`}
                  className="flex items-center justify-center w-10 h-10 rounded-full border border-border bg-card hover:bg-muted transition-colors"
                >
