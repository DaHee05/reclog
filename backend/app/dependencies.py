...
async def get_current_user(
    ...
    try:
        logger.info("=== AUTH: fetching JWKS ===")
        jwks = await _get_jwks()
        key = jwks["keys"][0]
        logger.info(f"=== AUTH: JWKS key alg={key.get('alg')}, kty={key.get('kty')} ===")
        payload = jwt.decode(
            token, key, algorithms=["ES256"], options={"verify_aud": False}
        )
        user_id_str: Optional[str] = payload.get("sub")
        logger.info(f"=== AUTH: decoded sub={user_id_str}, email={payload.get('email')} ===")
    ...
    logger.info(f"=== AUTH: user not found by id, checking email={email} ===")
    ...
    if email:
        ...
        if old_user:
            old_id = old_user.id
            logger.info(f"=== AUTH: migrating old user {old_id} -> {user_id} ===")
            ...
            logger.info(f"=== AUTH: migration done ===")
            return new_user
