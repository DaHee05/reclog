...
- 통계: 카테고리별 기록 수, 자주 쓴 태그 Top 10, 월별 히트맵 — 공유 스페이스 기록 제외
...
| **OpenAI gpt-image-1** | 서비스 내 AI 기능 — 포토북 이미지 위 손글씨 텍스트 합성 (런타임 사용) |
...
여행·일상 기록 앱은 많지만 **"같이 쌓아가는 기록"** 에 집중한 서비스는 드뭅니다. 커플·친구·가족이 하나의 스페이스에 각자의 시각으로 같은 순간을 기록하면, 나중에 보는 재미가 달라집니다. 여기에 AI 손글씨로 감성적인 포토북을 만드는 기능을 더해 **기록 → 공유 → 출력** 의 완성된 흐름을 제공합니다.

### 비즈니스 가능성

- **구독 모델**: 무료(AI 5회/일, 개인 기록) → 프리미엄(무제한 AI, 스페이스 다수 생성, 포토북 할인)
- **포토북 실물 출력**: 파트너 인쇄소 연동으로 수익화
- **기념일 알림**: 기록 날짜 기반 알림으로 리텐션 확보

### 시간이 더 있었다면

- **실시간 공동 편집**: WebSocket으로 스페이스 내 실시간 피드 업데이트
- **지도 시각화**: 기록 위치를 지도 핀으로 모아보기
- **Lv3 구현**: 관리자 대시보드, 결제 연동, 실물 포토북 주문 처리
- **Error Boundary**: 프론트엔드 전역 에러 처리 미비 개선 → 구현 완료
- **이미지 스토리지 보안**: 현재 Supabase public 버킷 사용 중 — 추후 private 버킷 + signed URL(만료 시간 포함)로 전환하여 URL 노출 시에도 일정 시간 후 무효화되도록 개선 예정
- **파일 업로드 검증**: 현재 Content-Type 헤더 기반 검증 — 추후 서버 측 magic bytes 검사로 강화 예정
